import copy
import calculation_lib_centralized


def value_iteration(Grid, S, A, R, gamma):
    """
    :param Grid: object of the class Environment
    :param S: set of all states
    :param A: Action set containing joint action tuple for each state
    :param R: Reward function
    :param gamma: discount factor

    :return V: value V for all joint states
    """
    V = {s: 0 for s in S}
    ...
    while True:
        ...
        for s in S:
            for a in A[s]:
                QQ = 0
                T = calculation_lib_centralized.get_transition_prob_centralized(Grid, s, a)  # returns {s:__, s': __}
                for ss in list(T.keys()):
                    QQ = QQ + T[ss] * (gamma * V[ss])
                Q[s][a] = R(s, a) + QQ
        ...
        if max(Residual[s] for s in S) < 0.001 or iterations >= 600:
            # print("Value Iteration for Agent " + agent.label + " is done after " + str(iterations) + " Iterations!!")
            break
    return V, PI


def blame_value_iteration(Grid, S, A, R_blame, gamma):
    """
    :param Grid: object of the class Environment
    :param S: set of all states
    :param A: Action set containing joint action tuple for each state
...
def action_set_value_iteration(agent, S):
    ...
    while True:
        ...
        for s in S:
            for a in agent.A[s]:
                QQ = 0
                T = calculation_lib_centralized.get_transition_prob_centralized(agent, s, a)
                for ss in list(T.keys()):
